...
@implementation Asn1Parser
{
	...
	WEAK_VARIABLE id <Asn1ParserDelegate> _delegate;
}
...
- (void)_parseErrorEncountered:(NSString *)errorMsg
{
	...
	_parserError = [NSError errorWithDomain:@"Asn1ParserDomain" code:1 userInfo:userInfo];
...
- (BOOL)_parseValueWithTag:(NSUInteger)tag dataRange:(NSRange)dataRange
{
	if (!dataRange.length)
	{
        ...
        {
            case Asn1TypeNull:
            case Asn1TypeTeletexString:
            case Asn1TypeGraphicString:
            case Asn1TypePrintableString:
            case Asn1TypeUtf8String:
            case Asn1TypeIA5String:
                break;
            default:
                return NO;
        }
	...
	switch (tag)
	{
		...
		case Asn1TypeBitString:
		{
			if (_delegateFlags.delegateSupportsBitString)
			{
				...
				Asn1BitString *bitstring = [[Asn1BitString alloc] initWithData:data unusedBits:unusedBits];
		...
		case Asn1TypeUtcTime:
		case Asn1TypeGeneralizedTime:
		{
...
- (BOOL)_parseRange:(NSRange)range
{
    ...
    do
	{
		...
		LLASN1Type tagType = tagByte & 31;
		BOOL tagConstructed = (tagByte >> 5) & 1;
		
		if (tagType == Asn1TypeUsesLongForm)
		{
...
- (id <Asn1ParserDelegate>)delegate
{
	return _delegate;
}


- (void)setDelegate:(id <Asn1ParserDelegate>)delegate;
{
	_delegate = delegate;
