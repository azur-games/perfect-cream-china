Shader "Playgendary/Shader+MULTIFUNCBackground"
{
    Properties
    {
        _ColorGradientBright ("Main Bright", Color) = (1,1,1,1)
        _ColorGradientDark ("Color Dark", Color) = (1,1,1,1)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            sampler2D _MainTex;
            fixed4 _ColorGradientBright;
            fixed4 _ColorGradientDark;
            float4 _MainTex_ST;     
            ...
            fixed4 frag(vertexOutput i) : SV_Target
            {
                ...
                float textureBrightness = tex2D(_MainTex, i.uv).r;
                float4 texColor = textureBrightness * _ColorGradientBright + (1.0f - textureBrightness) * _ColorGradientDark;
                resultColor = 1;// lerp(texColor, tex2D(_SecondMainTex, i.uv), _TransitionFactor);
                #else
                float textureBrightness = tex2D(_MainTex, i.uv).r;
                float4 texColor = textureBrightness * _ColorGradientBright + (1.0f - textureBrightness) * _ColorGradientDark;
                resultColor = texColor;// texColor;
                #endif
                ...
                #else
                resultColor.rgb *= LIGHT_ATTENUATION(i);                                
                #endif      
