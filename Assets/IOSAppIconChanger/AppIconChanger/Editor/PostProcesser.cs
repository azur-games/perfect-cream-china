...
namespace AppIconChanger.Editor
{
    public static class PostProcessor
    {
        ...
        public static void OnPostprocessBuild(BuildTarget target, string pathToBuiltProject)
        {
            ...
            string icon_names = string.Empty;

            foreach (var alternateIcon in alternateIcons)
            {
                if (string.IsNullOrEmpty(icon_names))
                {
                    icon_names = alternateIcon.iconName;
                }
                else
                {
                    icon_names += $" {alternateIcon.iconName}";
            ...
            pbxProject.SetBuildProperty(targetGuid, "ASSETCATALOG_COMPILER_ALTERNATE_APPICON_NAMES", icon_names);
