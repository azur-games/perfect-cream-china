...
public class UnicodeToKruti {
	...
	public static string Convert(string unicode_substring){
		int array_one_length = array_one.Length;
		string modified_substring = unicode_substring;

		int position_of_quote = modified_substring.IndexOf("'", StringComparison.Ordinal);
		while (position_of_quote >= 0){ 
			modified_substring =  ReplaceFirstOccurrence(modified_substring,"'","^");
			modified_substring =  ReplaceFirstOccurrence(modified_substring,"'","*");
			position_of_quote = modified_substring.IndexOf("'", StringComparison.Ordinal);
		}

		int position_of_Dquote = modified_substring.IndexOf("\"", StringComparison.Ordinal);
		while (position_of_Dquote >= 0){ 
			...
			position_of_Dquote = modified_substring.IndexOf("\"", StringComparison.Ordinal);
		}
		...
		for( int input_symbol_idx = 0; input_symbol_idx < array_one_length; input_symbol_idx++ )
		{
			...
			while (idx != -1 ) //whie-00
			{
				...
				idx = modified_substring.IndexOf (array_one [input_symbol_idx], StringComparison.Ordinal);
